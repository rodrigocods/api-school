...
export class StudentController {
   ...
   */
  async readStudents(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
    try {
   ...
   * @returns HTTP response
   */
  async readStudent(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
    try {
  ...
  async deleteStudent(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
    try {
      ...
      if(!studentID || !StringUtility.isJustNumber(studentID)) {
        return res.status(400).json({ error: 'Invalid request' });
      }
